...
contract TWAPUniswap{
    ...
    function update() external {
        (uint _price0Cumulative, uint _price1Cumulative, uint32 _blockTimeStamp) = UniswapV2OracleLibrary.currentCumulativePrices(address(pair));
        uint32 timeElapsed = _blockTimeStamp - blockTimeStampLast;
        require(timeElapsed >= PERIIOD, "Elapsed time < min");
        price0Average = FixedPoint.uq112x112(uint224(_price0Cumulative - price0CumulativeLast) / timeElapsed);
        price1Average = FixedPoint.uq112x112(uint224(_price1Cumulative - price1CumulativeLast) / timeElapsed);
    ...
    function getPrice(address token) external view returns (uint112 integer, uint112 decimal) {
        require(token == token0 || token == token1);
